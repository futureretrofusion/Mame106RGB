...
/* FRF_FIX74_CORE_PIPELINE_COUNTERS */
extern volatile UINT32 frf74_core_updatescreen_calls;
extern volatile UINT32 frf74_core_draw_screen_calls;
extern volatile UINT32 frf74_core_skip_calls;
extern volatile UINT32 frf74_core_update_video_calls;
extern volatile cycles_t frf74_core_draw_screen_ticks;
extern volatile cycles_t frf74_core_update_video_ticks;
...
void force_partial_update(int scanline)
{
	...
	if (clip.min_y <= clip.max_y)
	{
		...
		frf_perf_video_begin();
		(*Machine->drv->video_update)(0, scrbitmap[0], &clip);
		frf_perf_video_end();
		frf_perf_note_partial_update();
...
{
    /* FIX74: core frame entry */
    cycles_t frf74TickStart;
    int frf74Skipped;

    frf74_core_updatescreen_calls++;

    frf_perf_frame_begin();

    /* update sound */
	if(Machine->sample_rate>0 && soundMixerIsOn() && _frame>=_bootframeskip && (throttleIsOn==0))
    {
        frf_perf_sound_begin();
        sound_frame_update();
        frf_perf_sound_end();
    }
	...
	/* FIX74: one stable skip decision */
    frf74Skipped = osd_skip_this_frame();
    if(frf74Skipped)
        frf74_core_skip_calls++;

    if(!frf74Skipped)
	{
		profiler_mark(PROFILER_VIDEO);
		/* FIX74: draw_screen timing */
        frf74TickStart = osd_cycles();
        draw_screen();
        frf74_core_draw_screen_ticks +=
            osd_cycles() - frf74TickStart;
        frf74_core_draw_screen_calls++;
		profiler_mark(PROFILER_END);
	}
	...
	/* blit to the screen */
    frf_perf_osd_begin();
	/* FIX74: update_video_and_audio timing */
    frf74TickStart = osd_cycles();
    frf74_core_update_video_calls++;
    update_video_and_audio();
    frf74_core_update_video_ticks +=
        osd_cycles() - frf74TickStart;
    frf_perf_osd_end();
