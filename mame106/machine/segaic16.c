...
{
    struct compare_timer_chip *chip = &compare_timer[which];
    UINT16 old_counter = chip->counter;

    /* FRF_XBOARD_COMPARE_TIMER_FASTPATH_V1 */
    if (old_counter == 0xfff)
    {
        chip->counter = chip->regs[8] & 0xfff;
        return 1;
    }

    if (chip->regs[10] & 1)
        chip->counter = old_counter + 1;
