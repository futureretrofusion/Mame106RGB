...
/*
 * FRF Mame106RGB V3G FIX2 TAITO REPAIR
 *
 * Constant PC080SN rowscroll is represented as one tilemap row. Non-constant
 * rowscroll automatically retains the original 512-row implementation.
 */
static int frf_PC080SN_rows_mode[PC080SN_MAX_CHIPS][2];
static int frf_PC080SN_fastrows_setting = -1;
static int frf_PC080SN_fastrows_logged = 0;
static int frf_PC090OJ_skipzero_setting = -1;
static int frf_PC090OJ_skipzero_logged = 0;

static int frf_taito_env_value(const char *name,int default_value)
{
	const char *value = getenv(name);
	if (!value || !value[0]) return default_value;
	if (!strcmp(value,"0") || !strcmp(value,"OFF") || !strcmp(value,"off") ||
		!strcmp(value,"NO") || !strcmp(value,"no") ||
		!strcmp(value,"FALSE") || !strcmp(value,"false")) return 0;
	return 1;
}

static int frf_PC080SN_fastrows_enabled(void)
{
	if (frf_PC080SN_fastrows_setting < 0)
		frf_PC080SN_fastrows_setting =
			frf_taito_env_value("FRF_PC080SN_FASTROWS",1);
	return frf_PC080SN_fastrows_setting;
}

static int frf_PC090OJ_skipzero_enabled(void)
{
	if (frf_PC090OJ_skipzero_setting < 0)
		frf_PC090OJ_skipzero_setting =
			frf_taito_env_value("FRF_PC090OJ_SKIP_ZERO",0);
	return frf_PC090OJ_skipzero_setting;
}

static int frf_PC080SN_constant_rowscroll(int chip,int layer,UINT16 *constant)
{
	int j;
	UINT16 value = PC080SN_bgscroll_ram[chip][layer][0];
	for (j=1;j<256;j++)
		if (PC080SN_bgscroll_ram[chip][layer][j] != value)
			return 0;
	*constant = value;
	return 1;
}
				...
				int opaque,int dblwidth)
{
	...
	for (i = 0;i < chips;i++)
	{
		...
		if (!PC080SN_dblwidth)
		{
			...
			frf_PC080SN_rows_mode[i][0] = 512;
			frf_PC080SN_rows_mode[i][1] = 512;
		}
	}
...
void PC080SN_tilemap_update(void)
{
	...
	for (chip=0;chip<PC080SN_chips;chip++)
	{
		...
		if (!PC080SN_dblwidth)
		{
			...
			{
				if (frf_PC080SN_fastrows_enabled() &&
					frf_PC080SN_constant_rowscroll(chip,layer,&constant))
				{
					if (frf_PC080SN_rows_mode[chip][layer] != 1)
					{
						tilemap_set_scroll_rows(PC080SN_tilemap[chip][layer],1);
						frf_PC080SN_rows_mode[chip][layer] = 1;
					}
					tilemap_set_scrollx(
						PC080SN_tilemap[chip][layer],0,
						PC080SN_bgscrollx[chip][layer] - constant);

					if (!frf_PC080SN_fastrows_logged)
					{
						frf_PC080SN_fastrows_logged = 1;
						printf("FRF Mame106RGB V3G FIX2: PC080SN constant-row fast path active\n");
					}
				}
				else
				{
					if (frf_PC080SN_rows_mode[chip][layer] != 512)
					{
						tilemap_set_scroll_rows(PC080SN_tilemap[chip][layer],512);
						frf_PC080SN_rows_mode[chip][layer] = 512;
					}
					for (j=0;j<256;j++)
						tilemap_set_scrollx(
							PC080SN_tilemap[chip][layer],
							(j + PC080SN_bgscrolly[chip][layer]) & 0x1ff,
							PC080SN_bgscrollx[chip][layer] -
								PC080SN_bgscroll_ram[chip][layer][j]);
				}
			}
		}
		else
...
if (frf_PC090OJ_skipzero_enabled() &&
	((PC090OJ_ram_buffered[offs+0] |
	  PC090OJ_ram_buffered[offs+1] |
	  PC090OJ_ram_buffered[offs+2] |
	  PC090OJ_ram_buffered[offs+3]) == 0))
{
	if (!frf_PC090OJ_skipzero_logged)
	{
		frf_PC090OJ_skipzero_logged = 1;
		printf("FRF Mame106RGB V3G FIX2: PC090OJ zero-slot skip active\n");
	}
	continue;
}
...
/* Completely clipped sprites need no drawgfx call. */
if (x > cliprect->max_x || y > cliprect->max_y ||
	x + 15 < cliprect->min_x || y + 15 < cliprect->min_y)
	continue;
