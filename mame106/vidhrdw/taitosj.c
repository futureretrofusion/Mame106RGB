...
/*
 * FRF Mame106RGB V3G TAITOSJ COLLISION MODE
 *
 * 2 = original accurate sprite/sprite + sprite/playfield collision
 * 1 = retain sprite/sprite, skip expensive sprite/playfield bitmap work
 * 0 = skip both collision passes (benchmark only)
 *
 * Accurate mode remains the default.
 */
static int frf_taitosj_collision_mode(void)
{
	static int mode = -1;
	...
	{
		const char *value =
			getenv("FRF_TAITOSJ_COLLISION");

		mode = value ? atoi(value) : 2;
		...
		printf(
			"FRF Mame106RGB V3G TAITOSJ collision mode %d\n",
			mode);
	}
...
static void drawplayfield(int n, mame_bitmap *bitmap)
{
	...
	if (taitosj_video_enable & playfield_enable_mask[n])
	{
		...
		if (frf_taitosj_collision_mode() >= 2)
		{
...
VIDEO_UPDATE( taitosj )
{
	...
	if (frf_taitosj_collision_mode() >= 1)
