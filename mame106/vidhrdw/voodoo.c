...
int voodoo_start(int which, int type, int fbmem_in_mb, int tmem0_in_mb, int tmem1_in_mb)
{
	...
	v->type = type;
	v->chipmask = 0x01;
	/* FRF FIX61 V3: convert one Voodoo clock through the Amiga timer ABI */
	v->subseconds_per_cycle = DOUBLE_TO_SUBSECONDS(1.0 / (double)v->freq);
	...
	if (!v->pci.op_pending)
	{
		...
		if (cycles > 0)
		{
			...
			if (LOG_FIFO_VERBOSE) logerror("VOODOO.%d.FIFO:direct write start at %d.%08X%08X end at %d.%08X%08X\n", v->index,
				mame_timer_get_time().seconds, (UINT32)((UINT64)mame_timer_get_time().subseconds >> 32), (UINT32)mame_timer_get_time().subseconds,
				v->pci.op_end_time.seconds, (UINT32)((UINT64)v->pci.op_end_time.subseconds >> 32), (UINT32)v->pci.op_end_time.subseconds);
		}
	}
}
...
static void flush_fifos(voodoo_state *v, mame_time current_time)
{
	...
	if (LOG_FIFO_VERBOSE) logerror("VOODOO.%d.FIFO:flush_fifos start -- pending=%d.%08X%08X cur=%d.%08X%08X\n", v->index,
		v->pci.op_end_time.seconds, (UINT32)((UINT64)v->pci.op_end_time.subseconds >> 32), (UINT32)v->pci.op_end_time.subseconds,
		current_time.seconds, (UINT32)((UINT64)current_time.subseconds >> 32), (UINT32)current_time.subseconds);
