...
static UINT8 *deco32_dirty_palette;
...
WRITE32_HANDLER( deco32_palette_dma_w )
{
	...
	for (i=0; i<m; i++) {
		if (deco32_dirty_palette[i]) {
			deco32_dirty_palette[i]=0;
...
VIDEO_START( fghthist )
{
	...
	pf1a_tilemap =0;
	deco32_dirty_palette = auto_malloc(4096);
...
VIDEO_START( dragngun )
{
	...
	tilemap_set_transparent_pen(pf3_tilemap,0);
	tilemap_set_transparent_pen(pf1a_tilemap,0);
	memset(deco32_dirty_palette,0,4096);
