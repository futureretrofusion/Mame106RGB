...
static void brkthru_drawsprites( mame_bitmap *bitmap, const rectangle *cliprect, int prio )
{
	...
	/* Draw the sprites. It is important to draw them in this exact order
	   to preserve the hardware priority behaviour. */
	for (offs = 0;offs < spriteram_size; offs += 4)
	{
		if ((spriteram[offs] & 0x09) == prio)
		{
			...
			if (spriteram[offs] & 0x10)	/* double height */
			{
				...
				/* redraw with vertical wraparound */
				dgp0.code = code & ~1;
