...
/* FRF FIX42: restored Star Cruiser collision clip scope */
int starcrus_collision_check_s1s2(void)
{
	...
	struct drawgfxParams ship1_draw = {
		ship1_vid,
	...
	struct drawgfxParams ship2_draw = {
		ship2_vid,
	...
	ship1_draw.code = (s1_sprite & 0x03) ^ 0x03;
	ship1_draw.flipx = (s1_sprite & 0x08) >> 3;
	ship1_draw.flipy = (s1_sprite & 0x10) >> 4;
	ship1_draw.sx = s1_x - org_x;
	ship1_draw.sy = s1_y - org_y;
	drawgfx(&ship1_draw);

	ship2_draw.code = (s2_sprite & 0x03) ^ 0x03;
	ship2_draw.flipx = (s2_sprite & 0x08) >> 3;
	ship2_draw.flipy = (s2_sprite & 0x10) >> 4;
	ship2_draw.sx = s2_x - org_x;
	ship2_draw.sy = s2_y - org_y;
	drawgfx(&ship2_draw);
	...
	for (sy = 0; sy < 16; sy++)
	{
		for (sx = 0; sx < 16; sx++)
		{
			if (read_pixel(ship1_vid, sx, sy) == Machine->pens[1] &&
			    read_pixel(ship2_vid, sx, sy) == Machine->pens[1])
				return 1;
		}
	}
...
int starcrus_collision_check_s1p1p2(void)
{
	...
	struct drawgfxParams proj1_draw = {
		proj1_vid,
	...
	struct drawgfxParams proj2_draw = {
		proj2_vid,
	...
	if (p1_sprite & 0x08)
	{
		proj1_draw.code = (p1_sprite & 0x03) ^ 0x03;
		proj1_draw.sx = p1_x - org_x;
		proj1_draw.sy = p1_y - org_y;
		drawgfx(&proj1_draw);
	}

	if (p2_sprite & 0x08)
	{
		proj2_draw.code = (p2_sprite & 0x03) ^ 0x03;
		proj2_draw.sx = p2_x - org_x;
		proj2_draw.sy = p2_y - org_y;
		drawgfx(&proj2_draw);
	}
