...
WRITE8_HANDLER( popeye_bitmap_w )
{
	...
	if (bitmap_type == TYPE_SKYSKIPR)
	{
		...
		/* FRF Mame106RGB V3G POPEYE FASTBOX */
		plot_box(tmpbitmap2,sx,sy,8,8,colour);
	}
	else
	{
		sx = 8 * (offset % 64);
		...
		colour = Machine->pens[data & 0x0f];
		/* FRF Mame106RGB V3G POPEYE FASTBOX */
		plot_box(tmpbitmap2,sx,sy,8,4,colour);
	}
}
...
static void popeye_draw_background(mame_bitmap *bitmap, const rectangle *cliprect)
{
	...
	/*
	 * FRF Mame106RGB V3G POPEYE PALCACHE
	 *
	 * The selected PROM bank normally remains unchanged for many frames.
	 * Avoid calling palette_set_color() for all 16 background colours on
	 * every VIDEO_UPDATE.
	 */
	{
		static int frf_last_background_bank = -1;
		int frf_background_bank =
			(*popeye_palettebank & 0x08) >> 3;

		if(frf_background_bank !=
		   frf_last_background_bank)
		{
