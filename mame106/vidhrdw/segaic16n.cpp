...
/* FRF_XBOARD_SPRITE_CACHE_CULL_V1
 *
 * X-board sprite graphics ROM is immutable. Cache the end-marker scan for
 * sprite rows which repeat across frames, using a small direct-mapped table.
 */
struct frf_xboard_width_cache_entry
{
    UINT32 key;
    UINT16 words;
    UINT16 valid;
...
static struct frf_xboard_width_cache_entry
    frf_xboard_width_cache[1024];
static const UINT32 *frf_xboard_width_cache_base;

static inline FINLINE int frf_xboard_sprite_width_words(
    const UINT32 *spritebase,
    const UINT32 *spritedata,
    UINT16 addr,
    int bank)
{
    UINT32 key;
    UINT32 slot;
    struct frf_xboard_width_cache_entry *entry;
    int words;

    if (frf_xboard_width_cache_base != spritebase)
    {
        memset(frf_xboard_width_cache, 0,
               sizeof(frf_xboard_width_cache));
        frf_xboard_width_cache_base = spritebase;
    }

    key = ((UINT32)(bank & 0xff) << 16) | (UINT32)addr;
    slot = (key ^ (key >> 10) ^ (key >> 20)) & 1023;
    entry = &frf_xboard_width_cache[slot];

    if (entry->valid && entry->key == key)
        return entry->words;

    words = 0;
    while ((spritedata[addr + words] & 0x000000f0) != 0x000000f0)
        words++;
    words++;

    entry->key = key;
    entry->words = (UINT16)words;
    entry->valid = 1;
    return words;
}
...
void segaic16_sprites_outrun_drawT(struct sprite_info *info, mame_bitmap *bitmap, const rectangle *cliprect)
{
 ...
 numBankMask--;
    ...
    /* FRF_XBOARD_SPRITE_CACHE_CULL_V1 */
    int pixwords = frf_xboard_sprite_width_words(
        spritebase, spritedata, addr, bank);
    int pixwidth = (pixwords << 3) - 2;
    
    /* Reject a complete projected sprite before entering scanline loops. */
    int frf_ymin = (ydelta > 0) ? top : (ytarget + 1);
    int frf_ymax = (ydelta > 0) ? (ytarget - 1) : top;
    if (frf_ymax < cliprect->min_y ||
        frf_ymin > cliprect->max_y)
        continue;
    
    int frf_projected_width = (pixwidth * hzoom) >> 10;
    int frf_xend = xpos +
        ((xdelta > 0) ? frf_projected_width : -frf_projected_width);
    if ((xpos < cliprect->min_x - 2 &&
         frf_xend < cliprect->min_x - 2) ||
        (xpos > cliprect->max_x + 2 &&
         frf_xend > cliprect->max_x + 2))
        continue;
