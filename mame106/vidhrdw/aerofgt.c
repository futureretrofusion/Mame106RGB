...
{
	int offs;
	struct drawgfxParams dgp_aero = {
		bitmap,
		Machine->gfx[sprite_gfx],
		0, 0, 0, 0, 0, 0,
		cliprect,
		TRANSPARENCY_PEN,
		15,
	...
	{
		int attr_start = 4 * (aerofgt_spriteram3[offs] & 0x03ff);
		...
		if ((aerofgt_spriteram3[attr_start + 2] & 0x3000) == priority)
		{
			...
			for (y = 0;y <= ysize;y++)
			{
				...
				for (x = 0;x <= xsize;x++)
				{
					...
					dgp_aero.gfx = Machine->gfx[sprite_gfx + (map_start >= 0x2000 ? 1 : 0)];
					dgp_aero.code = code;
					dgp_aero.color = color;
					dgp_aero.flipx = flipx;
					dgp_aero.flipy = flipy;
					dgp_aero.sx = sx;
					dgp_aero.sy = sy;
					dgp_aero.scalex = zoomx << 11;
					dgp_aero.scaley = zoomy << 11;
					drawgfxzoom(&dgp_aero);
					map_start++;
				}
			}
		}
...
static void turbofrc_drawsprites(mame_bitmap *bitmap,const rectangle *cliprect,int chip,int chip_disabled_pri)
{
	...
	struct drawgfxParams dgp_turbo = {
		bitmap,
		Machine->gfx[sprite_gfx + chip],
		0, 0, 0, 0, 0, 0,
		cliprect,
		TRANSPARENCY_PEN,
		15,
	...
	for (attr_start = base + 0x0200-8;attr_start >= first + base;attr_start -= 4)
	{
		...
		for (y = 0;y <= ysize;y++)
		{
			...
			for (x = 0;x <= xsize;x++)
			{
				...
				dgp_turbo.code = code;
				dgp_turbo.color = color;
				dgp_turbo.flipx = flipx;
				dgp_turbo.flipy = flipy;
				dgp_turbo.sx = sx;
				dgp_turbo.sy = sy;
				dgp_turbo.scalex = zoomx << 11;
				dgp_turbo.scaley = zoomy << 11;
				dgp_turbo.priority_mask = (pri || chip == chip_disabled_pri) ? 0 : 2;
				drawgfxzoom(&dgp_turbo);
				map_start++;
			}
...
{
	int i;
	struct drawgfxParams dgp_pspikesb = {
		bitmap,
		Machine->gfx[sprite_gfx],
		0, 0, 0, 0, 0, 0,
		cliprect,
	...
	for (i = 4;i < aerofgt_spriteram3_size/2;i += 4)
	{
		...
		dgp_pspikesb.code = code;
		dgp_pspikesb.color = color;
		dgp_pspikesb.flipx = flipx;
		dgp_pspikesb.flipy = flipy;
		dgp_pspikesb.sx = xpos;
		dgp_pspikesb.sy = ypos;
		drawgfx(&dgp_pspikesb);

		/* wrap around y */
		dgp_pspikesb.sy = ypos + 512;
		drawgfx(&dgp_pspikesb);
	}
}
	...
	for (attr_start = aerofgt_spriteram3_size/2 - 4;attr_start >= 0;attr_start -= 4)
	{
		...
		for (y = 0;y <= ysize;y++)
		{
			...
			for (x = 0;x <= xsize;x++)
			{
				...
				dgp_boot.gfx = Machine->gfx[sprite_gfx + (code >= 0x1000 ? 0 : 1)];
				dgp_boot.code = code;
				dgp_boot.color = color;
				dgp_boot.flipx = flipx;
				dgp_boot.flipy = flipy;
				dgp_boot.sx = sx;
				dgp_boot.sy = sy;
				dgp_boot.scalex = zoomx << 11;
				dgp_boot.scaley = zoomy << 11;
				dgp_boot.priority_mask = pri ? 0 : 0x2;
				drawgfxzoom(&dgp_boot);
				code++;
			}
		}
	}
...
/* FRF FIX55: removed duplicate tail and restored Aerofighters sprite renderers */
