...
/* FRF FIX47: restored six SNK tile loops */
					...
					int x_size, int y_size, int bg_type)
{
	...
	struct drawgfxParams dgp0 = {
		tmpbitmap, gfx, 0, 0, 0, 0, 0, 0, NULL,
...
void tnk3_draw_text(mame_bitmap *bitmap, int bank, UINT8 *source)
{
	...
	struct drawgfxParams dgp1 = {
		bitmap, gfx, 0, 0, 0, 0, 0, 0, clip,
		TRANSPARENCY_PEN, 15, 0, 0, NULL, 0
	};
	...
	for (x = 0; x < 32; x++)
	{
		for (y = 0; y < 32; y++)
		{
			...
			if (tile_number == 0x20 || tile_number == 0xff)
				continue;
...
static void tdfever_draw_bg(mame_bitmap *bitmap, int xscroll, int yscroll)
{
	...
	for (x = 0; x < 32; x++)
	{
		for (y = 0; y < 32; y++)
		{
			    ...
			    attributes != dirtybuffer[offs + 1])
			{
				...
				{
					plot_box(tmpbitmap, sx, sy, gfx->width, gfx->height,
					         get_black_pen());
				}
				else
				{
	...
	copyscrollbitmap(bitmap, tmpbitmap, 1, &xscroll, 1, &yscroll,
	                 clip, TRANSPARENCY_NONE, 0);
}
...
static void tdfever_draw_tx(mame_bitmap *bitmap, int attributes,
                            int dx, int dy, int base)
{
