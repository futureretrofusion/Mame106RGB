...
VIDEO_START( vb )
{
	...
	/* FRF FIX65: fail cleanly before tile/sprite code dereferences missing state. */
	if (!vb_videoram || !vb_attribram || !vb_scrolly_lo ||
		!spriteram || spriteram_size < 4 ||
		!Machine->gfx[0] || !Machine->gfx[1] ||
		!prom || memory_region_length(REGION_PROMS) < 0x1000)
	{
		loginfo(2, "FRF FIX65: VBall video startup data is incomplete\n");
		return 1;
	}
	...
	bg_tilemap = tilemap_create(get_bg_tile_info, background_scan,
		TILEMAP_OPAQUE, 8, 8, 64, 64);
	if (!bg_tilemap)
		return 1;
...
static void draw_sprites(mame_bitmap *bitmap, const rectangle *cliprect)
{
	...
	/* FRF FIX65: canonical sprite path; no switch can bypass initialization. */
	if (!bitmap || !cliprect || !gfx || !src || spriteram_size < 4)
		return;

/*  240-Y    S|X|CLR|WCH WHICH    240-X
    xxxxxxxx x|x|xxx|xxx xxxxxxxx xxxxxxxx */
	for (i = 0; i + 3 < spriteram_size; i += 4)
	{
		int attr = src[i + 1];
...
INTERRUPT_GEN( vball_interrupt )
{
	...
	/* FRF FIX65: preserve the native 31..0 path but reject any bad scheduler value. */
	if (line >= 0 && line < 32)
		scrollx[31 - line] = vb_scrollx_hi + vb_scrollx_lo + 4;
}
