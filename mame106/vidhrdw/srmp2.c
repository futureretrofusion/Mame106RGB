...
/* FRF FIX46: restored sprite-map wraparound draws */
static void srmp3_draw_sprites_map(mame_bitmap *bitmap)
{
	...
	struct drawgfxParams dgp1 = {
		bitmap, Machine->gfx[0], 0, 0, 0, 0, 0, 0,
	...
	for (col = numcol - 1; col >= 0; col--)
	{
		...
		{
			int code = (((spriteram_3[((col) & 0x0f) * 0x40/2 + offs + 0x800/2] & 0xff) << 8) +
			            (spriteram_2[((col) & 0x0f) * 0x40/2 + offs + 0x800/2] & 0xff));
			...
			dgp1.sx = sx; dgp1.sy = sy; drawgfx(&dgp1);
			dgp1.sx = sx - 0x200; dgp1.sy = sy; drawgfx(&dgp1);
			dgp1.sx = sx; dgp1.sy = sy + 0x100; drawgfx(&dgp1);
			dgp1.sx = sx - 0x200; dgp1.sy = sy + 0x100; drawgfx(&dgp1);
		}
	}
}
...
static void mjyuugi_draw_sprites_map(mame_bitmap *bitmap)
{
	...
	UINT16 *src = spriteram16_2 +
	              (((ctrl2 ^ (~ctrl2 << 1)) & 0x40) ? 0x2000/2 : 0);
	...
	for (col = numcol - 1; col >= 0; col--)
	{
		...
		for (offs = 0; offs < 0x40/2; offs += 2/2)
		{
			int code = src[((col) & 0x0f) * 0x40/2 + offs + 0x800/2];
			int color = src[((col) & 0x0f) * 0x40/2 + offs + 0xc00/2];
			...
			dgp3.sx = sx; dgp3.sy = sy; drawgfx(&dgp3);
			dgp3.sx = sx - 0x200; dgp3.sy = sy; drawgfx(&dgp3);
			dgp3.sx = sx; dgp3.sy = sy + 0x100; drawgfx(&dgp3);
			dgp3.sx = sx - 0x200; dgp3.sy = sy + 0x100; drawgfx(&dgp3);
		}
	}
}
