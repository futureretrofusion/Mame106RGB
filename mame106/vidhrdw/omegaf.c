...
static void draw_sprites(mame_bitmap *bitmap,const rectangle *cliprect)
{
	...
	struct drawgfxParams dgp0={
		bitmap_sp, 	// dest
		Machine->gfx[3], 	// safe initial normal-sprite gfx
		0, 	// code
		0, 	// color
		0, 	// flipx
		0, 	// flipy
	...
	for (offs = 11 ;offs < spriteram_size; offs += 16)
	{
		...
		if (spriteram[offs + 2] & 2)
		{
			...
			if(sprite_overdraw_enabled && (color & 8))
			{
				...
				/* FRF FIX53: restored static-sprite size graphics selection */
				dgp0.gfx = Machine->gfx[big ? 4 : 3];
				dgp0.code = tile;
				dgp0.color = color;
				dgp0.flipx = flipx;
				dgp0.flipy = flipy;
			...
			else
			{
				...
				/* FRF FIX53: restored normal-sprite size graphics selection */
				dgp1.gfx = Machine->gfx[big ? 4 : 3];
				dgp1.code = tile;
				dgp1.color = color;
				dgp1.flipx = flipx;
				dgp1.flipy = flipy;
