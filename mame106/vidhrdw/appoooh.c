...
/* FRF FIX43: restored Appoooh sprite renderers and removed duplicate tail */
        ...
        unsigned char *sprite)
{
    ...
    struct drawgfxParams dgp0 = {
        dest_bmp, gfx, 0, 0, 0, 0, 0, 0,
        cliprect, TRANSPARENCY_PEN, 0, 0, 0, NULL, 0
    };

    for (offs = 0x20 - 4; offs >= 0; offs -= 4)
    {
        int sy = 240 - sprite[offs + 0];
        int code = (sprite[offs + 1] >> 2) + ((sprite[offs + 2] >> 5) & 7) * 0x40;
        int color = sprite[offs + 2] & 0x0f;
        int sx = sprite[offs + 3];
        int flipx = sprite[offs + 1] & 1;
        ...
        unsigned char *sprite)
{
    ...
    {
        int sy = 240 - sprite[offs + 0];
        int code = 0x200 + (sprite[offs + 1] >> 2) + ((sprite[offs + 2] >> 5) & 7) * 0x40;
        int color = sprite[offs + 2] & 0x0f;
        int sx = sprite[offs + 3];
        int flipx = sprite[offs + 1] & 1;
...
VIDEO_UPDATE( appoooh )
{
    ...
    if (priority == 0) tilemap_draw(bitmap, cliprect, fg_tilemap, 0, 0);
