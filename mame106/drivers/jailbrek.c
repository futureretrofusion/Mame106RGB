...
/* FRF_JAILBREK_RESTORE_PORT_CLOCKS_V2
 * Restores clock values previously used by this MAME 0.106 Amiga port.
 * VLM5030 remains disabled, isolating clock/watchdog behaviour from
 * the separate voice-chip fault.
 */
/* FRF_JAILBREK_VLM_STABILITY_V1_RETIRED_BY_V11
 * Historical note: VLM5030 was previously muted while the BSY loop was diagnosed.
 * V11 restores real VLM5030 speech with a bounded 16 KiB speech image.
 */
...
/* FRF_JAILBREK_REAL_VLM5030_V11
 *
 * Working baseline retained in the backup: V10.2 emulated only the VLM5030
 * BSY handshake and proved that the repeating first tone/watchdog reset was
 * caused by returning BSY=0 forever. V11 restores the real VLM5030 device,
 * while retaining a bounded 16 KiB speech address space and ROM image.
 */
...
{
    /* bit 0 may control latch direction as on related Konami hardware */
    VLM5030_ST((data >> 1) & 1);
...
static ADDRESS_MAP_START( jailbrek_map, ADDRESS_SPACE_PROGRAM, 8 )
	...
	AM_RANGE(0x6000, 0x6000) AM_READ(jailbrek_speech_r) /* VLM5030 BSY */
...
{
    REGION_SOUND1,
    0x4000      /* complete mirrored 27128 speech address space */
};

/* FRF_JAILBREK_SN76489A_MAME136U4_V9
 *
 * The GX507 PCB carries an SN76489AN. MAME changed Jail Break from
 * generic SN76496 to the verified SN76489A variant after the TI PSG
 * core rewrite.
 */
static const struct SN76496_config
    jailbrek_sn76489a_config =
{
    SN76496_CONFIG_MAGIC,
    SN76496_VARIANT_SN76489A
};
...
static MACHINE_DRIVER_START( jailbrek )
	...
	MDRV_CPU_ADD(M6809, 18432000/12)        /* 3 MHz ??? */
...
MDRV_FRAMES_PER_SECOND(60.606061)
	...
	MDRV_SOUND_CONFIG(jailbrek_sn76489a_config)
...
ROM_START( jailbrek )
	...
	ROM_REGION( 0x4000, REGION_SOUND1, 0 ) /* complete 16 KiB speech image */
	ROM_LOAD( "jailb8c.bin", 0x0000, 0x4000, CRC(0c8a3605) SHA1(d886b66d3861c3a90a1825ccf5bf0011831ca366) )
ROM_END
...
ROM_START( manhatan )
	...
	ROM_REGION( 0x4000, REGION_SOUND1, 0 ) /* mirrored 8 KiB speech EPROM */
