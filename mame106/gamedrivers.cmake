option(OPT_ALBA "" ON)
option(OPT_ALLIEDL "" ON)
option(OPT_ALPHA "" ON)
option(OPT_AMIGA "" ON)
option(OPT_ATARI "" ON)
option(OPT_ATLUS "" ON)
option(OPT_BFM "" ON)
option(OPT_BURGERTIME "Build BurgerTime without the complete Data East group" ON) # FRF_BURGERTIME_ONLY_OPTION_V1
...
option(OPT_DYNAX "" ON)
option(OPT_EDEVICES "" ON)
option(OPT_EOLITH "" ON)
option(OPT_EXCELENT "" ON)
option(OPT_EXIDY "" ON)
option(OPT_F32 "" ON)
option(OPT_FUUKI "" ON)
option(OPT_GAELCO "" ON)
option(OPT_GAMEPLAN "" ON)
option(OPT_GAMETRON "" ON)
option(OPT_GOTTLIEB "" ON)
option(OPT_GREYHND "" ON)
option(OPT_IGS "" ON)
option(OPT_IREM "" ON)
option(OPT_ITECH "" ON)
...
option(OPT_KONAMI "" ON)
option(OPT_MEADOWS "" ON)
option(OPT_MERIT "" ON)
option(OPT_METRO "" ON)
option(OPT_MIDCOIN "" ON)
option(OPT_MIDW8080 "" ON)
option(OPT_MIDWAY "" ON)
...
option(OPT_MISC "" ON)
option(OPT_NAMCO "" ON)
option(OPT_NAMCOS "" ON)
option(OPT_NASCO "" ON)
option(OPT_NEOGEO "" ON)
option(OPT_NICHIBUT "" ON)
option(OPT_NINTENDO "" ON)
option(OPT_NIX "" ON)
option(OPT_NMK "" ON)
option(OPT_OLYMPIA "" ON)
option(OPT_OMORI "" ON)
option(OPT_ORCA "" ON)
option(OPT_PACIFIC "" ON)
option(OPT_PACMAN "" ON)
option(OPT_PHOENIX "" ON)
option(OPT_PLAYMARK "" ON)
option(OPT_PSIKYO "" ON)
option(OPT_RAMTEK "" ON)
option(OPT_RARE "" ON)
option(OPT_SANRITSU "" ON)
option(OPT_SEGA "" ON)
option(OPT_SEIBU "" ON)
option(OPT_SETA "" ON)
...
option(OPT_TCH "" ON)
option(OPT_TECFRI "" ON)
option(OPT_TECHNOS "" ON)
option(OPT_TEHKAN "" ON)
option(OPT_THEPIT "" ON)
option(OPT_TOAPLAN "" ON)
option(OPT_TONG "" ON)
option(OPT_UNICO "" ON)
option(OPT_UNIVERS "" ON)
option(OPT_UPL "" ON)
option(OPT_VALADON "" ON)
option(OPT_VELTMJR "" ON)
option(OPT_VENTURE "" ON)
option(OPT_VSYSTEM "" ON)
option(OPT_YUNSUNG "" ON)
option(OPT_ZACCARIA "" ON)

# FRF_BURGERTIME_ONLY_BUILD_V1
option(OPT_JAILBREK "Compile Jail Break and Manhattan 24" ON)
# FRF_JAILBREK_COMPILE_BLOCK_V1_BEGIN
option(OPT_TIMEPILOT "Compile Time Pilot and Space Pilot" ON)
# FRF_TIMEPILOT_SPACEPILOT_COMPILE_V11_BEGIN
if(OPT_TIMEPILOT)
    add_compile_definitions(LINK_TIMEPILOT=1)
...
# FRF_TIMEPILOT_SPACEPILOT_COMPILE_V11_END

if(OPT_JAILBREK)
    add_compile_definitions(LINK_JAILBREK=1)
    list(APPEND MAME_DRIVERS_SRC
        drivers/jailbrek.c
        vidhrdw/jailbrek.c
        machine/konami.c
...
# FRF_JAILBREK_COMPILE_BLOCK_V1_END

if(OPT_BURGERTIME)
    add_compile_definitions(LINK_BURGERTIME=1)

    # OPT_DATAEAST already contains the same three source files.
    if(NOT OPT_DATAEAST)
        list(APPEND MAME_DRIVERS_SRC
            drivers/btime.c
            machine/btime.c
...
if(OPT_KONAMI)
	...
	list(APPEND MAME_DRIVERS_SRC
		...
		machine/jackal.c vidhrdw/jackal.c   
...
if(OPT_MINIDTEA)
	...
	list(APPEND CPU_DEFS
		...
		HAS_YM3812=1 	)
	list(APPEND CPU_DEFS
		HAS_H6280=1 HAS_M6502=1 HAS_M65C02=1 HAS_M68000=1 HAS_Z80=1 
	)
endif()
if(OPT_MINIKONAMI)
...
if(MCPU_PPC403)
	# FRF FIX60 V2: use portable PowerPC on Amiga/m68k
	list(APPEND MAME_CPU_SRC cpu/powerpc/ppc.c cpu/powerpc/ppc.h )
endif()
if(MCPU_PPC602)
	# FRF FIX60 V2: use portable PowerPC on Amiga/m68k
	list(APPEND MAME_CPU_SRC cpu/powerpc/ppc.c cpu/powerpc/ppc.h )
...
# FRF FIX60 V2: include MAME 0.106 Voodoo/Banshee core
list(APPEND MAME_DRIVERS_SRC vidhrdw/voodoo.c vidhrdw/voodoo.h )
# FRF FIX62: prefer full Nintendo DK source when both targets are enabled
if(OPT_JUSTDKONG AND OPT_NINTENDO)
	list(REMOVE_ITEM MAME_DRIVERS_SRC drivers/dkong_short.c)
endif()
