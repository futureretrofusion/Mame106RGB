...
int TripleBuffer_CSB::init()
{
    ...
    for(int i=0; i<3 ;i++)
    {
        ...
        // FRF RGBFAST V1: ScreenBuffer memory is not guaranteed to be clean.
        // Clear every plane once so unused borders/rows can never show garbage.
        BitMap *clearbm = sb._pScreenBuffer->sb_BitMap;
        if(clearbm)
        {
            ULONG planeBytes = (ULONG)clearbm->BytesPerRow * (ULONG)clearbm->Rows;
            for(int plane=0; plane<(int)clearbm->Depth && plane<8; ++plane)
            {
                if(clearbm->Planes[plane]) BltClear(clearbm->Planes[plane],planeBytes,0);
            }
        }
    }
