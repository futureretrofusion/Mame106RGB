...
extern "C" {
    void frfOptPaletteCycle(int direction);
}
    ...
    while((im = (struct IntuiMessage *) GetMsg(pMsgPort)))
    {
        ...
        switch(imclass)
        {
            ...
            if(!(imqual & IEQUALIFIER_REPEAT) )
            {
                ...
                /*
                 * FRF V3L PALETTE HOTKEY
                 * Amiga+F9: next palette. Amiga+F8: previous palette.
                 * Consume both press and release so MAME does not see F8/F9.
                 */
                if((imqual & (IEQUALIFIER_LCOMMAND | IEQUALIFIER_RCOMMAND)) &&
                   (finalkeycode == 0x58 || finalkeycode == 0x57))
                {
                    if(!(imcode & IECODE_UP_PREFIX))
                        frfOptPaletteCycle(finalkeycode == 0x58 ? 1 : -1);
