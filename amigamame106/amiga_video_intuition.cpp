 ...
 * Derive all native colour modes from the configured HAM screen family.
 * Monitor/timing/geometry bits are preserved.  RGB32 and GREYSCALE use the
 * plain native display ID at depth 5; only their palette/remap differs.
 */
extern int frf89_color_mode;
extern unsigned long frf89_effective_modeid;
...
static ULONG frf89_effective_display_id(ULONG configured)
{
    ...
    int mode = frf89_color_mode % 5;
    if(mode < 0) mode += 5;
    frf89_modeid_failed = 0;

    if(configured == INVALID_ID)
    {
        frf89_modeid_failed = 1;
        frf89_effective_modeid = configured;
        return configured;
    }
    ...
    switch(mode)
    {
        ...
        case 1: /* EHB64 */
            ...
            candidate =
                (configured & ~((ULONG)HAM_KEY | (ULONG)EXTRAHALFBRITE_KEY)) |
                (ULONG)EXTRAHALFBRITE_KEY;
            break;
    ...
    if(candidate == INVALID_ID || FindDisplayInfo(candidate) == NULL)
    {
        frf89_modeid_failed = 1;
        frf89_effective_modeid = configured;
        return configured;
    ...
    frf89_effective_modeid = candidate;
    return candidate;
}

static ULONG frf89_effective_screen_depth(ULONG configured)
{
    int mode = frf89_color_mode % 5;
    ULONG depth = configured;
    if(mode < 0) mode += 5;

    if(mode == 2 || mode == 3)
        depth = 5; /* RGB32 / GREYSCALE */
    else if(mode == 4)
        depth = 4; /* RGB16 */

    frf89_effective_depth = (int)depth;
    return depth;
}
// from driver.h
...
{
    /* FRF_FIX72_HAM6_INTERLEAVED_SCREEN
     *
     * HAM6 uses one contiguous 240-byte six-plane scanline when the bitmap
     * is interleaved. Other native modes retain their previous layout.
     */
    bool frfFix72Ham6Interleaved = false;
#ifdef DIPF_IS_HAM
    if(_ScreenDepthAsked == 6 && _ScreenModeId != INVALID_ID)
    {
        struct DisplayInfo frfFix72DisplayInfo;

        memset(
            &frfFix72DisplayInfo,
            0,
            sizeof(frfFix72DisplayInfo));

        if(GetDisplayInfoData(
                NULL,
                (UBYTE *)&frfFix72DisplayInfo,
                sizeof(frfFix72DisplayInfo),
                DTAG_DISP,
                _ScreenModeId) > 0)
        {
            frfFix72Ham6Interleaved =
                (frfFix72DisplayInfo.PropertyFlags &
                 DIPF_IS_HAM) != 0;
        }
    }
#endif
   ...
   // note: all this is regular OS intuition, no CGX
    ...
    if(_flags & DISPFLAG_USEHEIGHTBUFFER)
    {
 	...
 	_pScreen = OpenScreenTags( NULL,
			SA_DisplayID, frf89_effective_display_id(_ScreenModeId) ,
                        SA_Title, (ULONG)"MAME", // used as ID by promotion tools and else ?
                        SA_Width, _fullscreenWidth,
                        SA_Height,appliedHeight,
                        SA_Depth, frf89_effective_screen_depth(_ScreenDepthAsked) ,
//                        SA_Behind,TRUE,    /* Open behind */
                        SA_Quiet,TRUE,     /* quiet */
			SA_Type,CUSTOMSCREEN,
			SA_Interleaved,(_ScreenDepthAsked == 6) ? TRUE : FALSE, // FRF_NATIVE_EHB_INTERLEAVED_V5
			SA_Interleaved,(frfFix72Ham6Interleaved ? TRUE : FALSE), // FIX72 V2: contiguous HAM6 Chip RAM line
