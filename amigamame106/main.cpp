...
/* FRF86B_CLI_FRFLOG */
extern int frf86b_runtime_log_override;
...
int main(int argc, char **argv)
{
    /*
     * FRF86B_CLI_FRFLOG
     * Consume before MAME's own option parser.
     */
    {
        int frf86b_i = 1;
        while(frf86b_i < argc)
        {
            ...
            if(strcmp(argv[frf86b_i], "-frflog") == 0)
            {
                if(frf86b_i + 1 < argc)
                {
                    frf86b_runtime_log_override =
                        atoi(argv[frf86b_i + 1]) != 0 ? 1 : 0;
                    frf86b_remove = 2;
                }
                else
                {
                    frf86b_runtime_log_override = 1;
                    frf86b_remove = 1;
                }
            }
            else if(strncmp(argv[frf86b_i], "-frflog=", 8) == 0)
            {
                frf86b_runtime_log_override =
                    atoi(argv[frf86b_i] + 8) != 0 ? 1 : 0;
                frf86b_remove = 1;
            }
            ...
            {
                int frf86b_j;
                for(frf86b_j = frf86b_i;
                    frf86b_j + frf86b_remove < argc;
                    frf86b_j++)
                {
                    argv[frf86b_j] =
                        argv[frf86b_j + frf86b_remove];
                }
                argc -= frf86b_remove;
                argv[argc] = 0;
                continue;
