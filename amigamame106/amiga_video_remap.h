...
public:
    Paletted();
    virtual ~Paletted(){}
    virtual int needRemap() const { return _needFirstRemap; }
...
/*
 * FRF V3: private native 4/5/6-plane screen palette selected from the
 * currently rendered indexed frame. The mapping is rebuilt only when the
 * source palette is initially installed or genuinely changes.
 */
class Paletted_Screen8Optimized : public Paletted_Pens8
{
public:
    Paletted_Screen8Optimized(struct Screen *pScreen);
    void updatePaletteRemap(_mame_display *display) override;
    int needRemap() const override;

protected:
    // FRF V3D: native pens remain fixed after the first optimized build.
    std::vector<ULONG> _frfStableCentres;
    int _frfStableReady;
    int _frfStableRelinkLogged;

    // FRF V3L: EHB awareness and live palette generation tracking.
    int _frfEhbMode;
    ULONG _frfPaletteGenerationSeen;
    int _frfPaletteModeSeen;
};
