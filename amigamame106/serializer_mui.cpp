...
/*
 * FRF Mame106RGB V3J FIX2 REQUESTER CTOR REPAIR
 *
 * Replace the entire constructor region.  The original V3J patcher found the
 * first "{}" in a hook initializer instead of the actual constructor body,
 * leaving the old initializer tail behind.
 */
MUISerializer::LScreenModeReq::LScreenModeReq(
    MUISerializer &ser,
    ULONG_SCREENMODEID &value)
    : Level(ser)
    , _value(&value)
    , _ScreenModeStartHook()
    , _ScreenModeStopHook()
    , _ScreenModeFilterHook()
{
    ...
    _ScreenModeFilterHook.h_Entry =
        (RE_HOOKFUNC)
        &MUISerializer::LScreenModeReq::ScreenModeFilter;

    _ScreenModeFilterHook.h_Data = this;

    /* FRF_RTG_ALL_FULLSCREEN_MODES_V1: preserve local P96/CGX tags; remove only depth filtering. */
    _ScreenModeTags =
    {
...
ULONG MUISerializer::LScreenModeReq::ScreenModeFilter(
    struct Hook *hook REG(a0),
    ULONG modeid REG(a1),
    struct ScreenModeRequester *smreq REG(a2))
{
...
{
    if(!hook || !popasl || !hook->h_Data || !smreq) return 1;
    MUISerializer::LScreenModeReq *plevel = (MUISerializer::LScreenModeReq *)hook->h_Data;

    ULONG modeid = smreq->sm_DisplayID;
    ULONG depth = smreq->sm_DisplayDepth;
    int isHam6 = 0;
    int isEhb6 = 0;
    ...
    if(modeid != INVALID_ID)
    {
        ...
        LONG gotDims = GetDisplayInfoData(
            NULL,(UBYTE *)&dims,sizeof(dims),DTAG_DIMS,modeid);
        ...
        if(isHam6 || isEhb6)
            depth = 6;
        else if(gotDims > 0 && depth > (ULONG)dims.MaxDepth)
            depth = dims.MaxDepth;
    }

    plevel->_ScreenModeTags[SMT_DISPLAYID].ti_Data = modeid;
    ...
    if(isHam6)
        printf("FRF HAM6 requester selected mode $%08lx depth 6\\n",modeid);
    if(isEhb6)
        printf("FRF EHB6 requester selected mode $%08lx depth 6\\n",modeid);
...
void MUISerializer::LScreenModeReq::SetDisplayName(ULONG_SCREENMODEID &displayid)
{
    ...
    else
    {
        ...
        v = GetDisplayInfoData(
            NULL,(UBYTE *)&displayNameInfo,sizeof(displayNameInfo),
        ...
        if(isHam6)
            _strDisplay += "  [HAM6 OCS/ECS]";
        else if(isEhb6)
            _strDisplay += "  [EHB6 OCS/ECS: 32+32]";
        else if(displayid._depth <= 8 && displayid._depth > 0)
        {
            int nbc = 1 << displayid._depth;
            char t[12];
            snprintf(t,sizeof(t)," %dc",nbc);
            _strDisplay += t;
        }
    }
